        ...
        <div class="card">
            <div class="card-body">
                <form method="post" action="{{ route('admin.tunjangan-profesi.mutasi.store') }}" enctype="multipart/form-data">
                    ...
                    <div class="row mb-3">
                        ...
                        <div class="col-lg-10 col-md-9">
                            <select name="jenis" class="form-select form-select-sm {{ $errors->has('jenis') ? 'border-danger' : '' }}">
								...
								@foreach($jenis as $j)
                                <option value="{{ $j->id }}" data-status="{{ $j->status }}" {{ old('jenis') == $j->id ? 'selected' : '' }}>{{ $j->nama }}</option>
                                @endforeach
                            </select>
                            @if($errors->has('jenis'))
                    ...
                    <div id="form-status-1" class="d-none">
                        <div class="row mb-3">
                            ...
                            <div class="col-lg-10 col-md-9">
                                <select name="pegawai" class="form-select form-select-sm {{ $errors->has('pegawai') ? 'border-danger' : '' }}">
                                    <option value="" disabled selected>--Pilih--</option>
                                    @foreach($pegawai_non_aktif as $p)
                    ...
                    <div id="form-status-0" class="d-none">
                        <div class="row mb-3">
                            <label class="col-lg-2 col-md-3 col-form-label">Pegawai <span class="text-danger">*</span></label>
                            <div class="col-lg-10 col-md-9">
                                <select name="pegawai_aktif" class="form-select form-select-sm {{ $errors->has('pegawai_aktif') ? 'border-danger' : '' }}">
                                    <option value="" disabled selected>--Pilih--</option>
                                    @foreach($pegawai_aktif as $p)
                                    <option value="{{ $p->pegawai->id }}">{{ $p->pegawai->nama }} - {{ $p->pegawai->nip }}</option>
                                    @endforeach
                                </select>
                                @if($errors->has('pegawai_aktif'))
                                <div class="small text-danger">{{ $errors->first('pegawai_aktif') }}</div>
                                @endif
                            </div>
                        </div>
                    </div>
...
<script>
    ...
    Spandiv.Select2("select[name=pegawai_aktif]");
    Spandiv.Select2("select[name=angkatan]");
    Spandiv.Select2("select[name=jenis]");

    $(document).on("change", "select[name=jenis]", function() {
        var status = $("select[name=jenis]").find("option[value="+$(this).val()+"]").data("status");
        if(status == 1) {
            $("#form-status-1").removeClass("d-none");
            $("#form-status-0").addClass("d-none");
        }
        else {
            $("#form-status-1").addClass("d-none");
            $("#form-status-0").removeClass("d-none");
        }
        $("input[name=status]").val(status);
    });

    $(document).on("change", "#form-status-1 select[name=pegawai]", function() {
        var pegawai = $(this).val();
		var unit_id = $("#form-status-1 select[name=pegawai]").find("option[value="+pegawai+"]").data("unitid");
		var unit_nama = $("#form-status-1 select[name=pegawai]").find("option[value="+pegawai+"]").data("unitnama");
		$("#form-status-1 input[name=unit_id]").val(unit_id);
		$("#form-status-1 input[name=unit]").val(unit_nama);
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                $("#form-status-1 select[name=gaji_pokok]").html(html);
                Spandiv.Select2("#form-status-1 select[name=gaji_pokok]");
                $("#form-status-1 select[name=gaji_pokok]").val(response.id);
